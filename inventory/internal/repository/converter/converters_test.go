...
func TestConverters(t *testing.T) {
	...
	t.Run("Value to domain", func(t *testing.T) {
		tests := []struct {
			val      repoModel.Value
			expected model.Value
		}{
			{
				val:      repoModel.StringValue{Value: "test_string"},
				expected: model.StringValue{Value: "test_string"},
			},
			{
				val:      repoModel.Int64Value{Value: 42},
				expected: model.Int64Value{Value: 42},
			},
			{
				val:      repoModel.DoubleValue{Value: 36.6},
				expected: model.DoubleValue{Value: 36.6},
			},
			{
				val:      repoModel.BoolValue{Value: true},
				expected: model.BoolValue{Value: true},
			},
		}

		for _, test := range tests {
			res := ValueToDomain(test.val)
