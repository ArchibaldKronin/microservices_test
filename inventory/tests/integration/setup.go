...
import (
	...
	"github.com/ArchibaldKronin/microservices_test/platform/pkg/testcontainers"
	"github.com/ArchibaldKronin/microservices_test/platform/pkg/testcontainers/app"
	"github.com/ArchibaldKronin/microservices_test/platform/pkg/testcontainers/mongo"
	"github.com/ArchibaldKronin/microservices_test/platform/pkg/testcontainers/network"
...
const (
	...
	inventoryAppName    = "app-inventory"
...
func setupTestEnvironment(ctx context.Context, envVars map[string]string) *TestEnvironment {
	...
	// Переопределяем порт MongoDB для подключения к контейнеру из testcontainers
	appEnv[testcontainers.MongoPortKey] = "27017"
	...
	// waitStrategy := wait.ForLog("gRPC InventoryService server listening").
	// 	WithStartupTimeout(startupTimeout)
